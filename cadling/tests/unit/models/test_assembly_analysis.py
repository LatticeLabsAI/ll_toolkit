...
class TestAssemblyAnalysisModel:
    ...
    def test_model_has_required_attributes(self):
        ...
        assert hasattr(model, 'has_numpy')
        assert isinstance(model.has_pythonocc, bool)
        assert isinstance(model.has_numpy, bool)
    ...
    def test_model_call_with_empty_document(self):
        ...
        # Should have results if dependencies available
        if model.has_numpy:
...
class TestSubassemblyDetection:
    ...
    def test_detect_naming_pattern_subassemblies(self):
        """Test detection via naming patterns."""
        pytest.importorskip("numpy")
        ...
        graph = AssemblyGraph()
        graph.add_node("root", node_type="assembly", name="Root")
        graph.add_node("wheel_1", node_type="part", name="Wheel_Front_Left")
        graph.add_node("wheel_2", node_type="part", name="Wheel_Front_Right")
        graph.add_node("wheel_3", node_type="part", name="Wheel_Rear_Left")

        graph.root_id = "root"

        subassemblies = model.detect_subassemblies(graph)

        # Should find groups based on naming patterns
        assert len(subassemblies) >= 1
...
class TestMatingDetection:
    ...
    def test_detect_mating_surfaces_with_shapes(self):
        """Test mating detection with OCC shapes."""
        ...
        # Should detect proximity (boxes at origin will overlap)
