...
| **M3** | Train proof-of-life checkpoints | ◐ proof-of-life achieved (RL-optimized decode path) | RL on the **real OCC/cadquery dispose reward** drove the VAE's prior-sampling validity **6% → 82%** with diversity *rising* (6 → 65 distinct valid shapes; per-epoch curve 0.25→0.90, reward 0.69→0.92) — genuine learning, not mode collapse. Surfaced + fixed a pre-existing executor bug that pinned validity at 0% for *any* sequence. Real DeepCAD subset wired (`palapav/DeepCAD-DSL`); supervised warm-start tried and recorded as a KL-collapse **negative**. Full record: `docs/plans/2026-06-08-m3-llgen-checkpoints.md`. **Caveat:** the gain is on the RL-optimized `generate_for_training` decode; unifying it with the deployment `generate()` path (so the checkpoint is orchestrator-usable) is the active follow-up. |
