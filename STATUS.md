...
| **M3** | Train proof-of-life checkpoints | ✅ proof-of-life achieved (deployable) | RL on the **real OCC/cadquery dispose reward** drove the VAE's prior-sampling validity **4% → 98%** on the **deployment `generate()` path** (orchestrator-usable), with 63 distinct valid shapes (per-epoch 0.25→0.98, reward 0.69→0.95) — genuine learning, not mode collapse. Along the way: (a) surfaced + fixed a pre-existing executor bug that pinned validity at 0% for *any* command sequence; (b) found + fixed a decode-path divergence so RL gains show up at inference (extracted shared `_decode_and_sample`); (c) wired a real DeepCAD subset (`palapav/DeepCAD-DSL`); (d) supervised warm-start tried + recorded as a KL-collapse **negative**. Checkpoint reproduced via `python -m ll_gen.training.proof_of_life` (gitignored). Full record: `docs/plans/2026-06-08-m3-llgen-checkpoints.md`. |
