...
| **M3** | Train proof-of-life checkpoints | ◐ training loop proven; reward too permissive (reward-hacked) | RL on the **real OCC/cadquery dispose reward** drives BRepCheck-**validity 6% → 98%**, proving the full executor → dispose → reward → REINFORCE-gradient loop runs end-to-end (plus FR-G5 checkpoint loading). **But structural inspection exposes reward-hacking:** the heldout prompts carry no `target_dimensions`, so the reward is pure topological validity — which *accepts a single planar face*. ~90% of "valid" trained outputs are **non-solid** stacks of concentric circles; the **actual-solid rate barely moves (3% → 7%)**. So M3 proves the *machinery*, not real solid CAD generation. The shipped fixes — executor schema bridge (was pinning validity at 0% for any sequence), decode-path unification (`_decode_and_sample`), FR-G5 nested-checkpoint loading — are real and independently valuable. Real DeepCAD subset wired (`palapav/DeepCAD-DSL`); supervised warm-start = documented KL-collapse **negative**. **Needed next:** a reward that requires a closed solid + prompt/dimension conditioning. Full record: `docs/plans/2026-06-08-m3-llgen-checkpoints.md`. |
