...
def test_cli_generates_text_from_stl(
    tiny_lm_with_tokenizer_dir, sphere_stl_file
) -> None:
    env = dict(os.environ)
    ...
    result = subprocess.run(
        [
            ...
            str(_SCRIPT),
            "--model",
            tiny_lm_with_tokenizer_dir,
            "--mesh",
            sphere_stl_file,
            "--prompt",
            "describe this part <mesh>",
            "--max-new-tokens",
            "4",
            "--no-cropping",
            "--shape-depth",
    ...
    assert (
        result.stdout is not None and result.stdout.strip()
    ), "Expected CLI to produce non-empty stdout, but it was empty or whitespace."
    assert result.stdout.strip(), (
        "expected the CLI to print non-empty generated text, got " f"{result.stdout!r}"
    )
