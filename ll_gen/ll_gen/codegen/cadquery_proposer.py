"""CadQuery code generator wrapper for ll_gen.

This module wraps cadling's CadQueryGenerator to produce typed CodeProposal
...
try:
    ...
    logger.debug("cadling not available; CadQueryProposer will raise on use")


class CadQueryProposer:
    """Wraps cadling's CadQueryGenerator to produce typed CodeProposal objects.

    This class manages the lifecycle of cadling's CadQueryGenerator, handling
    ...
    Attributes:
        ...
        generator: The underlying cadling CadQueryGenerator instance.
    ...
    def __init__(self, config: Optional[CodegenConfig] = None) -> None:
        ...
        Side-effects:
            If cadling is available, creates a CadQueryGenerator instance.
        """
    ...
    def propose(
        ...
        Returns:
            ...
            - language set to CADQUERY
            - imports_required extracted from the code
            - syntax_valid pre-checked
        ...
        if self.generator is None:
            raise RuntimeError("CadQueryGenerator failed to initialize")
