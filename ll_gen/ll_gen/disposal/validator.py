...
def validate_shape(
    ...
    critical_findings = [
        f for f in report.findings if f.severity == ErrorSeverity.CRITICAL
    ]
    ...
    report.is_manifold = not any(
        f.error_code
        in ("BRepCheck_InvalidMultiConnexity", "BRepCheck_FreeEdge", "NonManifoldEdge")
        for f in report.findings
    )
    report.is_watertight = not any(
        f.error_code
        in ("BRepCheck_NotClosed", "BRepCheck_NotConnected", "ShellNotClosed")
        for f in report.findings
    )
