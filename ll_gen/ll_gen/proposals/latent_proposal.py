...
class LatentProposal(BaseProposal):
    ...
    face_edge_adjacency: dict[int, list[int]] | None = None
    ...
    @property
    def face_grid_resolution(self) -> tuple | None:
    ...
    def validate_shapes(self) -> list[str]:
        ...
        for i, g in enumerate(self.face_grids):
            if g.ndim != 3:
                errors.append(f"face_grids[{i}] has ndim={g.ndim}, expected 3")
            elif g.shape[2] != 3:
                errors.append(f"face_grids[{i}] last dim={g.shape[2]}, expected 3")

        for i, e in enumerate(self.edge_points):
            if e.ndim != 2:
                errors.append(f"edge_points[{i}] has ndim={e.ndim}, expected 2")
            elif e.shape[1] != 3:
                errors.append(f"edge_points[{i}] last dim={e.shape[1]}, expected 3")

        if self.face_bboxes is not None:
            if self.face_bboxes.ndim != 2 or self.face_bboxes.shape[1] != 6:
                errors.append(
                    f"face_bboxes shape={self.face_bboxes.shape}, " f"expected (F, 6)"
                )
        ...
        if self.edge_bboxes is not None:
            if self.edge_bboxes.ndim != 2 or self.edge_bboxes.shape[1] != 6:
                errors.append(
                    f"edge_bboxes shape={self.edge_bboxes.shape}, " f"expected (E, 6)"
                )
    ...
    def compute_bounding_box(self) -> np.ndarray | None:
