...
def test_load_dataset_records_rejects_both_sources(tmp_path) -> None:
    """--prompts-file and --dataset are mutually exclusive."""
    from ll_gen.training.run import load_dataset_records

    prompts = tmp_path / "p.jsonl"
    prompts.write_text('{"prompt": "x"}\n')
    with pytest.raises(ValueError, match="mutually exclusive"):
        load_dataset_records(str(prompts), "deepcad", None, None)
